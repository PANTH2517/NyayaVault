...
export class HttpNodeTransport implements INodeTransport {
  ...
  async sendMessage(
    ...
    try {
      ...
      const val = verifyNetworkEnvelope(responseEnvelope, this.nodeRegistry, this.chainId);
      if (!val.valid) {
  ...
  async handleIncomingHttpRequest(
    ...
    // Verify envelope validity
    const val = verifyNetworkEnvelope(envelope, this.nodeRegistry, this.chainId);
    if (!val.valid) {
